...
{
    // Create this Class to use A* Pathfinding!
    //       Use AIController_BreadthFirst as an example!

    // Define what a "Node" looks like in A*
    ...
    // There are more lists! What are they?
    ...
    private NodeRecord SmallestElement(List<NodeRecord> targetList)
    {
        ...
        foreach (NodeRecord nodeRecord in targetList)
        {
            ...
            h = Vector3.Distance(nodeRecord.node.tf.position, GameManager.instance.targetNode.tf.position);
            f = h + g;
