...
public class AIController_Other : AIController
{
    // Create this Class to use Another AI method of your choosing!
    //       Use AIController_BreadthFirst as an example!
    //       Note: This can be a different Obstacle Avoidance! 
    //             This can be an addition to A* ( A new way to find H, maybe?)
    //             It can be something new. Just be sure to explain how it works!

    // This AI will use DepthFirst which follows one path until the end to try
    // and find the goal as fast as possible without necesarily finding the
    // shortest path.
    //
    // I find the last item in the open list when calculating a path, so that I'm
    // always grabbing the most recent node and moving along one path.
    ...
    public override IEnumerator CalculatePath()
    {
        ...
        {
            // Find the last element in the open list
